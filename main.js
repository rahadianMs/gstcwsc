document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATA STRUCTURE (Added 'verified' status for evidence) ---
    const dbData = {
        userData: {
            firstName: 'Mochamad',
            lastName: 'Nalendra',
        ...
        evidenceHub: [
            { id: 1, name: 'SK_Tim_Pariwisata_2024.pdf', type: 'application/pdf', size: 1200000, verified: false },
            { id: 2, name: 'Bagan_Struktur_Organisasi.png', type: 'image/png', size: 850000, verified: false },
            { id: 3, name: 'RKA_Dinas_Pariwisata_2025.xlsx', type: 'application/vnd.ms-excel', size: 450000, verified: true },
            { id: 4, name: 'Daftar_Hadir_Pelatihan_Barista.pdf', type: 'application/pdf', size: 680000, verified: false },
            { id: 5, name: 'Laporan_Demografi_Pekerja_Hotel_XYZ.docx', type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', size: 1500000, verified: false },
            { id: 6, name: 'Dokumen_Inventaris_Cagar_Budaya.pdf', type: 'application/pdf', size: 2300000, verified: false },
            { id: 7, name: 'Perdes_No_5_Tahun_2023_Pelindungan_Situs.pdf', type: 'application/pdf', size: 950000, verified: true },
            { id: 8, name: 'SOP_Penggunaan_Listrik_Gedung.pdf', type: 'application/pdf', size: 400000, verified: false },
            { id: 9, name: 'Tagihan_Listrik_Jan-Des_2023.zip', type: 'application/zip', size: 5200000, verified: false },
        ],
        consultations: [
    ...
    function updateDashboardForRole() {
        if(isConsultant) {
            document.getElementById('dashboard-subtitle').textContent = "Welcome, Consultant! Here's an overview of client compliance.";
            // Future logic to load different users can be added here
        } else {
            document.getElementById('dashboard-subtitle').textContent = "Welcome! Here's your compliance progress for Tourism Destination XX.";
        }
    }
    ...
    // --- STANDARD COMPLIANCE PAGE LOGIC (Role-aware) ---
    ...
    function renderComplianceTab(tabId) {
        ...
        switch(tabId) {
            ...
            case 'evidenceHub':
                ...
                if(!isConsultant) { // Only users can upload
                ...
                document.querySelectorAll('.verify-evidence-btn').forEach(btn => btn.addEventListener('click', handleVerifyFile));
                break;
            case 'discussion':
    ...
    function renderIndicator(indicator) {
        const linkedEvidenceHTML = indicator.evidence.length > 0
            ? `<div class="space-y-2">${indicator.evidence.map(id => { 
                const file = dbData.evidenceHub.find(f => f.id === id); 
                if (!file) return '';
                const verificationBadge = file.verified 
                    ? `<span class="ml-auto text-xs font-medium text-green-700 bg-green-100 px-2 py-0.5 rounded-full">Verified</span>`
                    : `<span class="ml-auto text-xs font-medium text-yellow-700 bg-yellow-100 px-2 py-0.5 rounded-full">Pending</span>`;
                return `<div class="flex items-center bg-gray-100 p-2 rounded-md text-sm">${getFileIcon(file.name)}<span class="text-gray-800 font-medium">${file.name}</span>${verificationBadge}</div>`; 
            }).join('')}</div>`
    ...
    function renderEvidenceHub() {
        const filesHTML = dbData.evidenceHub.map(file => {
            const adminActions = isConsultant ? `
                <button data-file-id="${file.id}" class="verify-evidence-btn p-2 text-gray-400 hover:text-green-500 rounded-full hover:bg-gray-200" title="Verify File">
            ...
            const verificationStatus = file.verified 
                ? `<span class="text-xs font-semibold text-green-700">Verified</span>`
                : `<span class="text-xs font-semibold text-yellow-700">Pending</span>`;
            ...
            <div class="flex items-center p-3 bg-gray-50 rounded-md border">
                ...
                ${adminActions}
                <button data-file-id="${file.id}" class="delete-evidence-btn p-2 text-gray-400 hover:text-red-500 rounded-full hover:bg-gray-200" title="Delete File">
        ...
        const uploadSection = isConsultant ? '' : `
    ...
    function handleFileUpload(e) {
        ...
        Array.from(e.target.files).forEach(file => {
            dbData.evidenceHub.push({
                ...
                type: file.type,
                size: file.size,
                verified: false // New files are not verified by default
            });
        });
    ...
    function handleVerifyFile(e) {
        const fileId = parseFloat(e.currentTarget.dataset.fileId);
        const file = dbData.evidenceHub.find(f => f.id === fileId);
        if(file) {
            file.verified = !file.verified; // Toggle verification status
            showToast(`File "${file.name}" status updated.`);
            renderComplianceTab('evidenceHub');
        }
    }
    ...
    // --- CONSULTATION PAGE LOGIC (Role-aware) ---
                ...
                <div class="mt-6 border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea id="notulensi-input" rows="6" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" ${!isConsultant ? 'readonly':''}>${session.notulensi}</textarea>
                        </div>
                         <div>
                            <label for="action-plan-input" class="block text-sm font-medium text-gray-700">Tindak Lanjut / Action Plan</label>
                            <textarea id="action-plan-input" rows="6" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" ${!isConsultant ? 'readonly':''}>${session.actionPlan}</textarea>
                        </div>
                    </div>
    ...
    function renderNewSessionForm(){
        if(isConsultant){
            showToast("Only users can request a new session.");
            return;
        }
        const container=document.getElementById('session-details-container');container.innerHTML=`
    ...
    function renderUserProfile() {
        ...
        const userTitle = isConsultant ? "Consultant" : title;


        document.getElementById('header-user-name').textContent = fullName;
        document.getElementById('header-user-role').textContent = userTitle;
        ...
        document.getElementById('profile-sidebar-title').textContent = userTitle;
    ...
    function handleSaveEvidence() {
        ...
        const selectedEvidence = Array.from(document.querySelectorAll('.evidence-checkbox:checked')).map(cb => parseFloat(cb.value));
